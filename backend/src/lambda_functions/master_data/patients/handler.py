from data_models.patient import Patient
from data_models.practice_patient import PracticePatient
from lambda_libs.database_crud import select, create, update, delete
from json import dumps, loads
import logging

# Setup Logging
logging.basicConfig(
    level=logging.DEBUG,
    format="%(asctime)s.%(msecs)03d %(levelname)s %(module)s - %(funcName)s: %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
)
logger = logging.getLogger()

entity_class = Patient
related_entity_class = PracticePatient


def lambda_handler(event, context):
    print(event)

    if event["httpMethod"] == "GET":
        return select(
            event,
            entity_class,
            entity_class.select_required_params,
            entity_class.all_params_select,
        )
    elif event["httpMethod"] == "POST":
        response = create(
            event,
            entity_class,
            entity_class.create_required_fields,
            entity_class.create_allowed_fields,
        )
        # Parse the JSON string in event["body"] into a Python dictionary
        event_body = loads(event["body"])

        # Assuming response["body"] is also a JSON string, parse it
        response_body = loads(response["body"])

        # Add the patient_id field
        event_body["patient_id"] = response_body["id"]
        event_body["status"] = "Active"

        # Convert the modified dictionary back to a JSON string
        updated_body = dumps(event_body)

        # Update the event object with id and status
        updated_event = {
            "statusCode": 201,
            "headers": {"Access-Control-Allow-Origin": "*"},
            "body": updated_body,
        }

        # Create the related entity
        related_entity_response = create(
            updated_event,
            related_entity_class,
            related_entity_class.create_required_fields,
            related_entity_class.create_allowed_fields,
        )

        return related_entity_response
    elif event["httpMethod"] == "PUT":
        response = update(
            event,
            entity_class,
            entity_class.update_required_fields,
            entity_class.update_allowed_fields,
            entity_class.create_required_fields,
        )
        related_entity_response = update(
            event,
            related_entity_class,
            related_entity_class.create_required_fields,
            related_entity_class.create_allowed_fields,
        )
        return related_entity_response
    elif event["httpMethod"] == "DELETE":
        response = delete(event, entity_class, entity_class.delete_required_fields)
        return response
    else:
        method = event["httpMethod"]
        message = f"{method} method is not allowed"
        description = f"{message} on this resource"
        error = {
            "errorType": "InvalidHttpMethod",
            "errorMessage": message,
            "errorDescription": description,
        }
        response = {"statusCode": 405, "body": dumps(error)}
        return response
