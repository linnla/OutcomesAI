...
STAGE = "dev"
DYNAMODB_TABLE_NAME = f"outcomesai_drchrono_patients_{STAGE}"
URL_DRCHRONO_DATA = "https://drchrono.com/api/patients"
...
def lambda_handler(event, context):
    ...
    try:
        ...
        refresh_type = get_parameter(event, "refresh_type")
        ...
        drchrono_items = []
        # If its an incremental refresh, run for the last 3 days
        if refresh_type == "Incremental":
            three_days_ago = (datetime.today() - timedelta(days=5)).strftime("%Y-%m-%d")
            url = f"{URL_DRCHRONO_DATA}?since={three_days_ago}"
            drchrono_items = get_drchrono_data(
                url=url, ssm_parameter_name=ssm_parameter_name, region=region
            )
        else:
            drchrono_items = get_drchrono_data(
...
def transform_data(items, practice_id):
    try:
        ...
        for item in items:
            first_appt = item["date_of_first_appointment"]
            last_appt = item["date_of_last_appointment"]

            # Ignore patients when have to appointments
            if (first_appt is None or first_appt == "") and (
                last_appt is None or last_appt == ""
            ):
                continue
